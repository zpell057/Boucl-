...
def showLoopOuterCall(inputCoord,distanceWanted):
    showLoop(G,inputCoord,distanceWanted)
def showLoop(G,inputCoord,distanceWanted):
    ...
    finalLoop = pathToRandomDirectionPoint[3:] +pathToSecondDirection[1:]+ gettingBackToOrigin[1:-2]

    F = cutGraph(G,minX-kmToDeg(0.5),maxX+kmToDeg(0.5),minY-kmToDeg(0.5),maxY+kmToDeg(0.5))

    #plt.clf()
    fig, ax = ox.plot_graph_route(F, finalLoop, route_linewidth=6, node_size=0, bgcolor='k', route_color='cyan',show =False,save=True)
